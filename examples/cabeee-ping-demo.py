...
def run():
    ...
    # add links
    topo.addLink(x, y, delay='10ms', bw=10) # bw = bandwidth
    topo.addLink(y, z, delay='10ms', bw=10)
    '''
    ...
    # Start ping server
    info("Starting pings...\n")
    pingserver_log = open("{}/z/ndnpingserver.log".format(ndn.workDir), "w")
    getPopen(ndn.net["z"], "ndnpingserver {}".format(PREFIX), stdout=pingserver_log,\
             stderr=pingserver_log)

    # start ping client
    ping1 = getPopen(ndn.net["w"], "ndnping {} -c 5".format(PREFIX), stdout=PIPE, stderr=PIPE)
    ping1.wait()
    printOutput(ping1.stdout.read())

    interface = ndn.net["y"].connectionsTo(ndn.net["x"])[0][0]
    info("Failing link\n") # failing link by setting link loss to 100%
    interface.config(delay="10ms", bw=10, loss=100)
    info ("\n starting ping2 client \n")

    ping2 = getPopen(ndn.net["w"], "ndnping {} -c 5".format(PREFIX), stdout=PIPE, stderr=PIPE)
    ping2.wait()
