...
def run():
    ...
    makespanNS = 0 # for now we don't use any CPU to run the services.
    cmd = BIN_DIR + '/cabeee-dag-serviceA-app {} {} {} > cabeee_serviceA_service1.log &'.format(PREFIX, "/service1", makespanNS)
    ndn.net['rtr3'].cmd(cmd)
    cmd = BIN_DIR + '/cabeee-dag-serviceA-app {} {} {} > cabeee_serviceA_service2.log &'.format(PREFIX, "/service2", makespanNS)
    ndn.net['rtr1'].cmd(cmd)
    cmd = BIN_DIR + '/cabeee-dag-serviceA-app {} {} {} > cabeee_serviceA_service3.log &'.format(PREFIX, "/service3", makespanNS)
    ...
    cmd = BIN_DIR + '/cabeee-dag-serviceA-app {} {} {} > cabeee_serviceA_service4.log &'.format(PREFIX, "/service4", makespanNS)
    ndn.net['rtr2'].cmd(cmd)
    sleep(1) # wait so that we don't start two applications on the same node at the same time (RIB update messages can get messed up, and only one service will properly register FIB)
    cmd = BIN_DIR + '/cabeee-dag-serviceA-app {} {} {} > cabeee_serviceA_service5.log &'.format(PREFIX, "/service5", makespanNS)
    ndn.net['rtr3'].cmd(cmd)
    cmd = BIN_DIR + '/cabeee-dag-serviceA-app {} {} {} > cabeee_serviceA_service6.log &'.format(PREFIX, "/service6", makespanNS)
    ndn.net['rtr1'].cmd(cmd)
    cmd = BIN_DIR + '/cabeee-dag-serviceA-app {} {} {} > cabeee_serviceA_service7.log &'.format(PREFIX, "/service7", makespanNS)
    ndn.net['rtr2'].cmd(cmd)
    sleep(1) # wait so that we don't start two applications on the same node at the same time (RIB update messages can get messed up, and only one service will properly register FIB)
    cmd = BIN_DIR + '/cabeee-dag-serviceA-app {} {} {} > cabeee_serviceA_service8.log &'.format(PREFIX, "/service8", makespanNS)
    ndn.net['rtr2'].cmd(cmd)
    ...
    cmd = BIN_DIR + '/cabeee-custom-app-consumer2 {} {} {} > cabeee_consumer2.log &'.format(PREFIX, WORKFLOW1, 1)
    ...
    # this is a new name so that the orchestrator listens to it: "/orchA/serviceOrchestration/reset"
    ...
    # App input is the main PREFIX, the workflow file, and the orchestration value (0, 1 or 2)
    cmd = BIN_DIR + '/cabeee-custom-app-consumer {} {} {} > cabeee_consumer.log &'.format(PREFIX, WORKFLOW2, 1)
