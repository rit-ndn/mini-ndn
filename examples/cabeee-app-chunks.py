...
def run():
    ...
    #testFileRX = "/tmp/cabeee-test-chunks-received"
    testFileTX = "/tmp/minindn/cabeee-test-chunks-to-transmit"
    cmd = 'echo "cabeee - demonstrate file transfer using an NDNApp!" > {}'.format(testFileTX)
    ...
    cmd = '/home/cabeee/mini-ndn/examples/apps/dist/cabeee_chunks_producer {} {} > cabeee_chunks_producer.log &'.format(PREFIX, testFileTX)
    producer  = ndn.net['sensor']
    producer.cmd(cmd)

    # or one-liner
    #ndn.net['sensor'].cmd("/home/cabeee/mini-ndn/examples/apps/dist/cabeee_chunks_producer")

    # choice 2: (looks like this is how it was done by Peter Kietzmann (Mini-NDN mail list August 2021), but it seems
    #           I need to create a consumer/producer/forwarder class that I call from here with the AppManager
    #           and then in that class, I use Application.start (just like it was done with nfd and nlsr)
    #           This ends up doing the same thing, but it takes more files to get it done (needs the "wrapper" class file)
    #       myProducer = AppManager(ndn, ndn.net['sensor'], <consumer/producer/forwarder class name>, logLevel="DEBUG")
    #       in that class:
    #       Application.start(self, '/dist/cabeee_producer', self.logFile)
    ...
    cmd = '/home/cabeee/mini-ndn/examples/apps/dist/cabeee_chunks_consumer {} > cabeee_chunks_consumer.log &'.format(PREFIX)
